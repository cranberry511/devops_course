...
resource "null_resource" "web_hosts_provision" {
  ...
  depends_on = [yandex_compute_instance.my_storage, yandex_compute_instance.web, yandex_compute_instance.db]
  ...
  provisioner "local-exec" {
    # without secrets
    command = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i ${abspath(path.module)}/hosts6.ini"
